...
<body>
    ...
    <div class="col-md-2" style="margin-left: 2%;">
        ...
        <?php if ($id_almacen == 350) { ?>
            <button type="button" class="btn btn-sm btn-warning" onclick='vaciar_almacen(<?php echo $id_almacen; ?>);'>
                <span class="glyphicon glyphicon-warning-sign"></span> Vaciar Almacen</button>
        <?php } ?>
    </div>
        ...
        <div class="card-body p-0 outer_div" id="tabla-completa">
            ...
            ?>
                    <tr>
                       ...
                       ?></td>
                        ...
                        <td style="background-color:<?php echo $bgColor;?>"> 
                         ...
                         <?php 
                              ...
                              if ($is_admin == 1 ) {
                        echo "<span style='display:none;'>".$dato['caducidad']."</span>"; 
                           echo '<input type="text" class="form-control" id="caducidad_'.$dato['id'].'" name="caducidad" value="'.$dato['caducidad'].'" readonly ondblclick="edit_item('.$dato['id'].', \'caducidad_'.$dato['id'].'\')" onfocusout="save_item('.$dato['id'].',4)">';
                        }else{
                                 echo $dato['caducidad']; 
                       ...
                       echo '<input type="text" class="form-control" id="cantidad_'.$dato['id'].'" name="cantidad" value="'.$dato['cantidad'].'" readonly ondblclick="edit_item('.$dato['id'].', \'cantidad_'.$dato['id'].'\')" onfocusout="save_item('.$dato['id'].',2)">';
                        }else{
                                 echo $dato['cantidad']; 
  ...
  var element=document.getElementById(item);
   ...
   var valor = document.getElementById('cantidad_'+id).value;
        element.removeAttribute('readonly');
            console.log('editando '+item+' id '+id);
            console.log(valor);
            sessionStorage.setItem('cantidad_vieja', valor);
...
case 2:
  ...
  var tipo = 'cantidad';
  var cantidad_vieja = sessionStorage.getItem('cantidad_vieja');
break;
...
default:
  ...
  $.ajax({
    ...
    data: "id="+id+"&tipo="+tipo+"&item="+item+"&cantidad_vieja="+cantidad_vieja,
...
function vaciar_almacen(id_almacen){
    if (confirm("¿Estás seguro de que deseas vaciar el almacén "+id_almacen+"? Esta acción no se puede deshacer.")) {
        $.ajax({
            type: "POST",
            url: "./ajax/vaciar_almacen.php",
            data: { id_almacen: id_almacen },
            beforeSend: function() {
                alert("Vaciar almacen en proceso...");
            },
            success: function(response) {
                alert("Almacén vaciado correctamente.");
                location.reload();
            },
            error: function() {
                alert("Hubo un error al vaciar el almacén. Por favor, inténtalo de nuevo.");
            }
        });
    }
}
