...
$id_usuario_actual = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 1; 
...
if (isset($_FILES['archivo_csv']) && $_FILES['archivo_csv']['error'] == 0) {
    ...
    while (($datos = fgetcsv($archivo, 1000, ",")) !== FALSE) { 
        list($referencia, $lote, $cantidad, $almacen, $folio) = $datos;
        ...
        if ($producto) {
            try {
                ...
                if ($cantidad_anterior <= 0 || $cantidad_anterior < $cantidad) {
                    $estado = "Cantidad insuficiente";
                }else {
                ...
                $stmtInsert = $pdo->prepare(
                    "INSERT INTO movimientos (id_producto, cantidad, tipo_movimiento, id_usuario, date_created, campo_actualizado, cantidad_anterior,folio) 
                     VALUES (?, ?, ?, ?, NOW(), ?, ?, ?)"
                );
                $stmtInsert->execute([
                    $producto['id'],
                    $cantidad_actual,          // La cantidad que queda en inventario
